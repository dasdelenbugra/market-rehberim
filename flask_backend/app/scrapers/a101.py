...
class A101Scraper(BaseScraper):
    ...
    def search_url(self, query: str) -> str:
        return f"{self.BASE}/arama/?k={quote(query)}"

    def parse(self, soup: BeautifulSoup) -> list[Item]:
        items: list[Item] = []
        for card in soup.select("div.product, li.product-item"):
            name_el = card.select_one(".name, .product-name, h3")
            price_el = card.select_one(".price, .current-price")
